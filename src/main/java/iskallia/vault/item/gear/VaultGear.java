...
public interface VaultGear<T extends Item> extends net.minecraftforge.common.extensions.IForgeItem {
    ...
    default void tickRoll(T item, ItemStack stack, World world, Entity entity, int itemSlot, boolean isSelected) {
        ...
        if ((int) displacement != lastModelHit) {
            ...
            ModAttributes.GEAR_MODEL.create(stack, world.rand.nextInt(numberOfModels));
            ModAttributes.GEAR_COLOR.create(stack, DyeColor.WHITE.getColorValue());
            if (item == ModItems.ETCHING) {
    ...
    default void addInformation(T item, ItemStack stack, World world, List<ITextComponent> tooltip, ITooltipFlag flag) {
        ...
        if (stack.getItem() != ModItems.SWORD && stack.getItem() != ModItems.DAGGER && stack.getItem() != ModItems.AXE && stack.getItem() != ModItems.ETCHING)
        { 
            Integer modelId = ModAttributes.GEAR_MODEL.getOrDefault(stack, -1).getValue(stack);
            ...
            if (rarity != Rarity.SCRAPPY) {
                ...
                if (gearModel != null && rarity != Rarity.SCRAPPY) {
                    tooltip.add(new StringTextComponent("Style: ").append(new StringTextComponent(gearModel.getDisplayName()).mergeStyle(rarity.color)));
                }
            }
        ...
        ModAttributes.GEAR_STATE.get(stack).map(attribute -> attribute.getValue(stack)).ifPresent(state -> {
            ...
            ModAttributes.GEAR_ROLL_TYPE.get(stack).map(attribute -> attribute.getValue(stack)).ifPresent(roll -> {
                if(roll.name() == "SCRAPPY")
                {
                    
                    String rarityName = roll.name().toLowerCase();
                    rarityName = rarityName.substring(0, 1).toUpperCase() + rarityName.substring(1);

                    tooltip.add(new StringTextComponent("Roll: ").append(new StringTextComponent(rarityName).mergeStyle(Rarity.SCRAPPY.color)).append(new StringTextComponent("+").mergeStyle(Rarity.SCRAPPY.color))); 
                
                }
                else if(roll.name() == "COMMON")
                {
                    
                    String rarityName = roll.name().toLowerCase();
                    rarityName = rarityName.substring(0, 1).toUpperCase() + rarityName.substring(1);

                    tooltip.add(new StringTextComponent("Roll: ").append(new StringTextComponent(rarityName).mergeStyle(Rarity.COMMON.color)).append(new StringTextComponent("+").mergeStyle(Rarity.COMMON.color))); 
                
                }
                else if(roll.name() == "RARE")
                {
                    
                    String rarityName = roll.name().toLowerCase();
                    rarityName = rarityName.substring(0, 1).toUpperCase() + rarityName.substring(1);

                    tooltip.add(new StringTextComponent("Roll: ").append(new StringTextComponent(rarityName).mergeStyle(Rarity.RARE.color)).append(new StringTextComponent("+").mergeStyle(Rarity.RARE.color))); 
                
                }
                else if(roll.name() == "EPIC")
                {
                    
                    String rarityName = roll.name().toLowerCase();
                    rarityName = rarityName.substring(0, 1).toUpperCase() + rarityName.substring(1);

                    tooltip.add(new StringTextComponent("Roll: ").append(new StringTextComponent(rarityName).mergeStyle(Rarity.EPIC.color)).append(new StringTextComponent("+").mergeStyle(Rarity.EPIC.color))); 
    ...
    default String getArmorTexture(T item, ItemStack itemStack, Entity entity, EquipmentSlotType slot, String type) {
        ...
        if (rarity == Rarity.SCRAPPY) {
            return ModModels.GearModel.SCRAPPY.getTextureName(slot, type, modelId);
        }
        ...
        return gearModel.getTextureName(slot, type, modelId);
    }
    ...
    enum RollType {
        SCRAPPY_ONLY, TREASURE_ONLY, ALL, SCRAPPY, COMMON, RARE, EPIC;
