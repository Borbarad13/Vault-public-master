...
public class ItemTraderCore extends Item {
    ...
    public static ItemStack generate(String nickname, int value, boolean megahead, CoreType type) {
        ...
        switch (type) {
            case BLACKMARKET:
                trades = ModConfigs.TRADER_CORE_BLACK_MARKET.TRADES.stream().filter(Trade::isValid).collect(Collectors.toList());
                break;
            case OMEGA:
    ...
    public static ItemStack getStackFromCore(TraderCore core, CoreType type) {
        ...
        switch (type) {
            ...
            case RAFFLE:
                ...
                break;
            case BLACKMARKET:
                stack = new ItemStack(ModItems.TRADER_CORE_BLACK_MARKET, 1);
                break;
            default:
        ...
        if (player.isSneaking()) {
            ...
            if (nbt.contains("core", Constants.NBT.TAG_COMPOUND)) {
                ...
                if (core.getTrade() == null) {
                    ...
                    if(stack.getItem() == ModItems.TRADER_CORE_BLACK_MARKET)
                    {
                        newTraderCore = generate(name, 1, false, CoreType.BLACKMARKET);
                    }
                    else
                    {
                        newTraderCore = generate(name, 1, false, CoreType.COMMON);
                    }
                ...
                if(stack.getItem() == ModItems.TRADER_CORE_BLACK_MARKET)
                {
                    newTraderCore = generate("Trader", 1, false, CoreType.BLACKMARKET);
                }
                else
                {
    ...
    public enum CoreType {
        ...
        BLACKMARKET(new StringTextComponent(TextFormatting.BLACK + "Black Market")),
