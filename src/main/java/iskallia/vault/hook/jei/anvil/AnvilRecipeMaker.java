	...
	public static Collection<Object> getAnvilRecipes(IVanillaRecipeFactory vanillaRecipeFactory)
	{
		...
		for (List<ItemStack> armorItemList : vaultArmorList)
		{
			...
			for (ItemStack armorItem : armorItemList)
			{
				...
				VaultGear.addLevel(result, 1.0F);
		...
		ArrayList<ItemStack> Pog = Lists.newArrayList(new ItemStack(ModItems.POG));
		...
		for (List<ItemStack> armorItemList : vaultArmorList)
		{
			...
			// Create anvil recipe.
			recipeList.add(vanillaRecipeFactory.createAnvilRecipe(inputList, Pog, resultList));
		}
		...
		List<ItemStack> cleansingCloth = Lists.newArrayList(new ItemStack(ModItems.CLEANSING_CLOTH));
		List<ItemStack> unstableVoidCore = Lists.newArrayList(new ItemStack(ModItems.UNSTABLE_VOID_CORE));
		List<ItemStack> crystalEngraver = Lists.newArrayList(new ItemStack(ModItems.CRYSTAL_ENGRAVER));
		...
		for (ItemStack crystal : vaultCrystalList)
		{
			...
			recipeList.add(vanillaRecipeFactory.createAnvilRecipe(crystal, cleansingCloth, Lists.newArrayList(result)));
			recipeList.add(vanillaRecipeFactory.createAnvilRecipe(crystal, crystalEngraver, Lists.newArrayList(result)));
			recipeList.add(vanillaRecipeFactory.createAnvilRecipe(crystal, unstableVoidCore, Lists.newArrayList(result)));
		}
...
//		for (int i = 0; i < gear.getModelsFor(VaultGear.Rarity.OMEGA); i++)
		{
			...
			IntegerAttribute.Generator max_level = new IntegerAttribute.Generator();
			...
			ModAttributes.GEAR_MAX_LEVEL.create(itemStack, random, max_level);
