...
public class EffectModifier extends VaultModifier {
	...
	public void tick(ServerWorld world, PlayerEntity player) {
		...
		VaultRaid raid = VaultRaidData.get(world).getActiveFor((ServerPlayerEntity)player);
		...
		raid.modifiers.forEach((i, modifier) -> {
			...
			//player.addPotionEffect(new EffectInstance(getEffect(), 20 * 20, newAmplifier));

			if (newAmplifier >= 0) { 
                EffectInstance newEffect = new EffectInstance(getEffect(), 20 * 20, newAmplifier, false, true);
            ...
            //world.getServer().getPlayerList().func_232641_a_(text, ChatType.CHAT, player.getUniqueID());
		});

		

		/*if(newAmplifier >= 0) {
			EffectInstance newEffect = new EffectInstance(this.getEffect(), 100, newAmplifier - 1,
